...
const hatimRoutes = require('./routes/hatims')
...
log(
  `boot: NODE_ENV=${process.env.NODE_ENV || ''} PORT=${process.env.PORT || ''} PASSENGER_APP_PORT=${process.env.PASSENGER_APP_PORT || ''} API_PORT=${process.env.API_PORT || ''}`
)
try {
  if (!fs.existsSync(logDir)) fs.mkdirSync(logDir, { recursive: true })
  fs.writeFileSync(
