...
router.post('/magic-link', async (req, res) => {
  try {
    ...
    await db.execute('UPDATE magic_links SET used = TRUE WHERE user_id = ? AND used = FALSE', [userId])
...
router.post('/verify', async (req, res) => {
  try {
    ...
    // Mark token as used
    await db.execute('UPDATE magic_links SET used = TRUE WHERE id = ?', [link.id])

    // Generate JWT
    const jwtToken = jwt.sign({ userId: link.user_id }, process.env.JWT_SECRET, {
