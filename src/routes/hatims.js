...
router.post('/:id/join', authMiddleware, async (req, res) => {
  try {
    ...
    const [hatims] = await db.execute('SELECT * FROM hatims WHERE id = ? AND status = ?', [
      hatimId,
      'active',
    ])
...
router.post('/join/:code', authMiddleware, async (req, res) => {
  try {
    const [hatims] = await db.execute('SELECT id FROM hatims WHERE invite_code = ? AND status = ?', [
      req.params.code,
      'active',
    ])
...
router.post('/:id/assign-juz', authMiddleware, async (req, res) => {
  try {
    ...
    const [juz] = await db.execute('SELECT * FROM juz_assignments WHERE hatim_id = ? AND juz_number = ?', [
      hatimId,
      juz_number,
    ])
...
router.post('/:id/complete-juz', authMiddleware, async (req, res) => {
  try {
    ...
    if (incomplete[0].count === 0) {
      ...
      await db.execute('UPDATE hatims SET status = ? WHERE id = ?', ['completed', hatimId])
    }
